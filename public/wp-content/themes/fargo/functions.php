<?php
  ...
  function fargo_gallery_shortcode($attrs) {
    global $post;
    ...
    foreach ($attachments as $id => $attachment) {
      if ($id != $post_thumbnail_id) {
        ...
        $imgtag = wp_get_attachment_image($id, 'thumbnail');
        $title = esc_attr($attachment->post_title);
        $output .= "<li>\n";
        $output .= "<a href=\"{$src[0]}\" rel=\"gallery\" class=\"colorbox\" title=\"{$title}\">{$imgtag}</a>\n";
        $output .= "</li>\n";
      }
    }
  ...
  add_filter('post_gallery', 'fargo_gallery_shortcode');
